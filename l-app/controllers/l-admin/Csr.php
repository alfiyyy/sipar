...
class Csr extends Backend_Controller {
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->csr_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						$row[] = $val['csr_id'];
						$row[] = $val['csr_nama'];
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->csr_model->datatable('_data', 'count')]);
				}
		...
		if ($this->role->i('write') ) 
		{
			if ($this->input->method() == 'post')
			{
				...
				if ($this->form_validation->run())
				{
					...
					if ($this->csr_model->insert($data_isert))
					{
	...
	public function edit($id_data = '')
	{
		if ($this->role->i('modify'))
		{
			...
			$cek_id = $this->csr_model->cek_id($id_edit);
			...
			if ($cek_id == 1) 
			{
				if ($this->input->method() == 'post')
				{
					...
					if ( $this->form_validation->run() )
					{
						...
						if ($this->csr_model->update($id_edit, $data_update))
						{
				...
				$data_edit = $this->csr_model->get_data_edit($id_edit);
	...
	public function pilih($id = '')
	{
		...
		if ( !empty($id) && $this->input->is_ajax_request() ) 
		{
			...
			$result = $this->csr_model->get_csr($id);
			$_output = $result;
			$_output['csr_nama'] = $result['csr_nama'];
			$_output['csr_nomor'] = $result['csr_nomor'];
			$_output['csr_id'] = $result['csr_id'];
			$_output['unker_id'] = $result['unker_id'];
			$_output['formhidden'] = '<input type="hidden" name="csr_id" value="'.$result['csr_id'].'" /> <input type="hidden" name="unker_id" value="'.$result['unker_id'].'" />';
			...
			<div class="card card-body">
				<div class="media">
					...
					<div class="media-body">
						<h6 class="tx-sans tx-uppercase tx-10 tx-spacing-1 tx-color-03 tx-semibold tx-nowrap mg-b-5 mg-md-b-8">'.$result['csr_nama'].'</h6>
						<h4 class="tx-20 tx-sm-18 tx-md-20 tx-normal tx-rubik mg-b-0">'.$result['csr_nomor'].'</h4>
					</div>
				</div>
			</div>
		</div>';
