	...
	<div class="card">
        <div class="card">
            <div class="card-header">
                <h6 class="lh-5 mg-b-0">Pencarian Booking Kamar</h6>
            </div>
            <?php 
			...
			<div class="card-body">
                ...
                <div class="box-body">
                    <div class="row">
                        <?php 
                        foreach ($room as $row) { 
                        $query_trx = $this->db->query('SELECT * FROM t_booking_room
                        WHERE room_id="'.$row['room_id'].'" AND (br_tanggal_in BETWEEN "'.$tgl_in.'" AND "'.$tgl_out.'")');
                        $result_trx = $query_trx->row_array();
                        if($result_trx['br_tanggal_in'] == NULL) {
                            $warna = 'success';
                        } else {
                            if($result_trx['br_status'] == "Belum Disetujui") {
                                $warna = 'primary';
                            } else {
                                $warna = 'danger';
                            }
                        ...
                        <div class="col-sm-6 col-md-3 mb-3">
                            <div class="card card-body">
                                <div class="media">
                                    <a href="#" <?php if($warna=="danger")  {} elseif($warna=="success") { ?> onclick="onModalBook(<?= $row['room_id'] ?>)" <?php } else { ?> onclick="window.location='<?=admin_url('riwayat-booking');?>'" <?php } ?> data-toggle="modal"><div class="wd-40 wd-md-50 ht-40 ht-md-50 bg-<?= $warna ?> tx-white mg-r-10 mg-md-r-10 d-flex align-items-center justify-content-center rounded"><i class="fa fa-bed"></i></div></a>
...
<div id="modal_booking" class="modal">
	<div class="modal-dialog">
		<div class="modal-content">
            <?=form_open('l-admin/booking-kamar/add');?>
			<div class="modal-header">
				<h5 class="modal-title">Booking Kamar</h5>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <div>
                                <input type="text" name="br_jml_hari" class="form-control" placeholder="Contoh : 2" require />
                            </div>
                        </div>
                    </div>
                </div>
