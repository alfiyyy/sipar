...
function onModalBook(id) {
	...
	$.ajax({
		type: 'POST',
		url: admin_url + 'room' + '/pilih/' + idDet,
		data:{
			'id': idDet,
			'csrf_name': csrfToken
		},
...
function onModalTambahPeserta(kategori_id) {
	...
	$('#kategori_show').html('<input type="hidden" name="kategori_id" id="kategori_id" value="'+kid+'" class="form-control" require />  <input type="hidden" name="aksi" value="room" />');
}
