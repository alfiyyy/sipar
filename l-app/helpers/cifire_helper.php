...
if (!function_exists('get_setting')) {
     ...
     * Example usage: get_setting($param = 'web_name')
     * 
     * @param string $param
     * @param bool $array Whether to return the whole array or just a single value
     * @param string $arr_param The specific parameter to return if $array is true
    ...
    function get_setting($param = '', $array = false, $arr_param = 'value') {
        ...
        if (!$array) {
            ...
            $query = $CI->db->where('options', $param)->get('t_setting');
            if ($query->num_rows() == 1) {
                $result = $query->row_array();
                $data = $result[$arr_param];
            }
        }
