...
class Smr_model extends CI_Model {
	...
	private $_column_order = array(null, 'smr_id', 'smr_nama', 'smr_nomor', 'created_at', 'smr_id');
	private $_column_search = array('smr_id', 'smr_nama', 'smr_nomor', 'created_at', 'smr_id');
	...
	*/
	...
	public function get_smr($id) 
	{
		if ( $this->cek_id($id) == 1 )
		{
			$result = $this->db->where('smr_id',$id)->get($this->_table)->row_array();
			return $result;
		}
		else
		{
	...
	private function _data_laporan()
	{
		...
		$this->db->select('t_smr.*, t_smr_kategori.kategori_nama, t_unit_kerja.unker_id');
		$this->db->from($this->_table);
		
		$this->db->join('t_smr_kategori', 't_smr.kategori_id = t_smr_kategori.kategori_id');
		$this->db->join('t_unit_kerja', 't_smr_kategori.unker_id = t_unit_kerja.unker_id');
		
		$this->db->where('t_unit_kerja.unker_id', $row->unker_id);
		$this->db->group_by('t_smr.smr_id'); 
		$this->db->order_by('t_smr.kategori_id, t_smr.smr_nama, t_smr.smr_nomor', 'ASC');
