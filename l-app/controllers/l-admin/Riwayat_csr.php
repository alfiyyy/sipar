...
class Riwayat_csr extends Backend_Controller {
    public $mod = 'riwayat-csr';
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->booking_csr_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						$row[] = $no;
						$row[] = $val['peserta_nip'];
						...
						// $row[] = $val['unker_nama'];
						$row[] = $val['csr_nama'].' / '.$val['csr_nomor'];
						...
						$row[] = '<div class="text-right"><div class="btn-group">
									<button type="button" onclick="location.href=\''. admin_url($this->mod."/setujui/".$val['bc_id']) .'\'" class="btn btn-md btn-success" data-toggle="tooltip" data-placement="top" data-title="Klik Untuk Menyetujui"><i class="cificon licon-check"></i></button>
									<button type="button" onclick="location.href=\''. admin_url($this->mod."/edit/".$val['bc_id']) .'\'" class="btn btn-md btn-primary" data-toggle="tooltip" data-placement="top" data-title="'. lang_line('button_edit') .'"><i class="cificon licon-edit"></i></button>
									<button type="button" class="btn btn-md btn-danger delete_single" data-toggle="tooltip" data-placement="top" data-title="'.lang_line('button_delete').'" data-pk="'. encrypt($val['bc_id']) .'"><i class="cificon licon-trash-2"></i></button>
								</div></div>';
						} else {
						$row[] = '<div class="text-right"><div class="btn-group">
							<button type="button" onclick="location.href=\''. admin_url($this->mod."/edit/".$val['bc_id']) .'\'" class="btn btn-md btn-primary" data-toggle="tooltip" data-placement="top" data-title="'. lang_line('button_edit') .'"><i class="cificon licon-edit"></i></button>
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->booking_csr_model->datatable('_data', 'count')]);
				}
