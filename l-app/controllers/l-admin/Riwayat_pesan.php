...
class Riwayat_pesan extends Backend_Controller {
    public $mod = 'riwayat-pesan';

	public function __construct() 
	{
		parent::__construct();
		
		$this->load->model("mod/pesanan_kamar_model");
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->pesanan_kamar_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						$row[] = $no;
						$row[] = $val['peserta_nip'];
						...
						if($val['kategori_nama']=='Kamar VIP') {
							$row[] = '<span id="cstatus_'.$val['br_id'].'" class="badge badge-outline-primary">'.$val['kategori_nama'].'</span><br />'.$val['unker_nama'];
						} else {
							$row[] = '<span id="cstatus_'.$val['br_id'].'" class="badge badge-outline-success">'.$val['kategori_nama'].'</span><br />'.$val['unker_nama'];
						}
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->pesanan_kamar_model->datatable('_data', 'count')]);
				}
