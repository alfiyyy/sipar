...
class Smr_layanan extends Backend_Controller {
	
	public $mod = 'smr-layanan';
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->smr_layanan_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						$row[] = $val['smrl_id'];
						$row[] = $val['smrl_nama'];
						...
						$row[] = '<div class="text-center"><div class="btn-group">
									<button type="button" onclick="location.href=\''. admin_url($this->mod."/edit/".$val['smrl_id']) .'\'" class="btn btn-xs btn-white" data-toggle="tooltip" data-placement="top" data-title="'. lang_line('button_edit') .'"><i class="cificon licon-edit"></i></button>
									<button type="button" class="btn btn-xs btn-white delete_single" data-toggle="tooltip" data-placement="top" data-title="'.lang_line('button_delete').'" data-pk="'. encrypt($val['smrl_id']) .'"><i class="cificon licon-trash-2"></i></button>
								</div></div>';
						$data[] = $row;
					} // endforeach.

					$this->json_output(['data' => $data, 'recordsFiltered' => $this->smr_layanan_model->datatable('_data', 'count')]);
				}
	...
	public function edit($id_data = '')
	{
		if ($this->role->i('modify'))
		{
			...
			$cek_id = $this->smr_layanan_model->cek_id($id_edit);
			...
			if ($cek_id == 1) 
			{
				if ($this->input->method() == 'post')
				{
					...
					if ( $this->form_validation->run() )
					{
						$data_update = array(
							...
							'smrl_nama' => xss_filter($this->input->post('smrl_nama')),
							'smrl_harga' => xss_filter($this->input->post('smrl_harga'), 'xss'),
							'created_at' => xss_filter($this->input->post('created_at')),
							'updated_at' => xss_filter($this->input->post('updated_at')),
						);

						if ($this->smr_layanan_model->update($id_edit, $data_update))
						{
				...
				$data_edit = $this->smr_layanan_model->get_data_edit($id_edit);
		...
		if ($this->input->is_ajax_request())
		{
			if ($this->role->i('delete'))
			{
				...
				foreach ($data as $key)
				{
					...
					$this->smr_layanan_model->delete($pk);
				}
