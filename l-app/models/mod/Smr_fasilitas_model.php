...
class Smr_fasilitas_model extends CI_Model {

	public $vars;
	private $_table = 't_smr_fasilitas';
	private $_column_order = array(null, 'smrf_id', 'smrf_nama', 'smrf_icon', 'smrf_id');
	private $_column_search = array('smrf_id', 'smrf_nama', 'smrf_icon', 'smrf_id');
	...
	*/
	...
	{
		$query = $this->db->where('smrf_id', $key);
	...
	public function delete($val = 0)
	{
		...
		{
			$this->db->where('smrf_id', $val)->delete($this->_table);
			return TRUE;
		}
		else
		{
	...
	{
		$query = $this->db->where('smrf_id', $val_id);
