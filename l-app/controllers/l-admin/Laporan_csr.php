...
class Laporan_csr extends Backend_Controller {
    public $mod = 'laporan-csr';
    ...
    public function index()
	{
		if ($this->role->i('read'))
		{
			...
			// $query2 = $this->db->query('SELECT uk.id_user, rk.* FROM t_csr_kategori rk
            ...
            $this->db->select('t_csr.*');
            $this->db->from('t_csr');
            $this->db->join('t_unit_kerja', 't_csr.unker_id = t_unit_kerja.unker_id');
            $this->db->join('t_user', 't_unit_kerja.id_user = t_user.id');
            
            $this->db->where('t_user.id', $id_user);
            $query = $this->db->order_by('t_csr.csr_nama, t_csr.csr_nomor', 'ASC');
			$query = $this->db->get();  
