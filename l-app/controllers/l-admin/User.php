			...
			if ( $this->input->is_ajax_request() && $this->role->i('write'))
			{
				$this->form_validation->set_rules(array(
					...
					array(
						...
						'rules' => 'required|trim|min_length[4]|max_length[200]|alpha_numeric_spaces',
					),
					array(
						'field' => 'birthday',
				...
				if ( $this->form_validation->run() ) 
				{
					...
					// if (!mkdir('l-content/uploads/dokumen/'.xss_filter($this->input->post('username')), 0777, true)) {
					...
					if ( $this->form_validation->run() ) 
					{
						...
						if ($in_pass2 != null) {
							$dataUpdate = array(
								...
								'password' => $password,
								'email'    => $this->input->post('email', TRUE),
								'username' => xss_filter($this->input->post('username'), 'xss'),
								'name'     => xss_filter($this->input->post('name'), 'xss'),
							...
							$verif = $this->input->post('status_accept');
							...
							if ($verif == 0) {
								$dataUpdate = array(
									...
									'tlpn'     => xss_filter($this->input->post('tlpn'), 'xss'),
									'active'   => xss_filter($this->input->post('active'), 'xss'),
									'status_terima'   => xss_filter($this->input->post('status_accept'), 'xss')
								);
							} elseif ($verif == 1) {
								$dataUpdate = array(
									'key_group' => xss_filter($this->input->post('group'), 'xss'),
									'password' => encrypt(xss_filter($this->input->post('username'))),
									'email'    => $this->input->post('email', TRUE),
									'username' => xss_filter($this->input->post('username'), 'xss'),
									'name'     => xss_filter($this->input->post('name'), 'xss'),
									'gender'   => $this->input->post('gender',TRUE),
								...
								$subject = 'Pendaftaran SIPAR Diterima';
								$message = '<html><body><p>Hai '.$this->input->post('name').', Pendaftaran SIPAR kamu telah diverifikasi, kamu bisa login menggunakan :<p><br><p>USERNAME : '.$this->input->post('username').'</p><p>PASSWORD : '.$this->input->post('username').'</p></body></html>';
								$name    = explode('@', $mail)[0];
						...
						//echo xss_filter($this->input->post('username')); die();
