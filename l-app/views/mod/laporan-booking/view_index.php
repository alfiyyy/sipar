...
<div class="page-inner">
		...
		<div class="pageheader pd-t-20 pd-b-0">
			<div class="d-flex justify-content-between">
				<div class="clearfix">
					...
					<h4 class="pd-0 mg-0 tx-20">Laporan Kamar</h4>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="card">
        <div class="card">
            ...
            <?php 
                echo form_open('','method="GET" autocomplete="off" class="form-bordered"');
            ...
            <div class="card-footer">
                ...
                <a href="#" class="btn btn-lg btn-warning mr-2"><i class="cificon licon-file mr-2"></i>Cetak PDF</a>
            </div>
            <?=form_close();?>
        </div> <!-- card -->
				...
				<table class="table table-hover table-striped table-bordered table-datatable">
					...
					<tbody>
                        ...
                        <tr>
                            <td><?= $no ?></td>
                            <?php
                            if($row['kategori_nama']=='Kamar VIP') {
                                echo '<td><span id="cstatus_'.$row['kategori_id'].'" class="badge badge-primary">'.$row['kategori_nama'].'</span></td>';
                            } else {
                                echo '<td><span id="cstatus_'.$row['kategori_id'].'" class="badge badge-success">'.$row['kategori_nama'].'</span></td>';
                            }
                            ?>
                            <td><?= $row['room_nama'] ?></td>
                            <td><?= $row['room_nomor'] ?></td>
                            <?php for ($x=1; $x<=31; $x++) {
                                $y = $tahun;
                                $b = $bulan;
                                $tgl = strlen($x);
                                ...
                                $query1 = $this->db->select('*')->from('t_booking_room');
                                $query1 = $this->db->where('room_id', $row['room_id']);
                                $query1 = $this->db->where('br_tanggal_in', $tanggal_array);
                                $query1 = $this->db->get();  
